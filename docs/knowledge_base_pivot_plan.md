...
**状态：** 本方案的 P0–P4 全部完成并合入；转型已落地。后续工作已超出本文档范围，见下方 §0.1。
**分支：** `feat/knowledge-base-pivot`
**日期：** 2026-06-26（方案定稿）· 2026-07-31（状态更新）
**作者：** （规划讨论）

> 这是一份**历史规划文档**，记录转型当时的判断与取舍，不是当前功能的说明书。
> 产品现状请看 [README](../README.md) / [中文 README](../README.zh-CN.md)。

---

## 0.1 本方案之后的演进（摘要）

方案定稿后，实现继续推进并超出了原范围。以下能力**不在**本文档的 P0–P4 里，但已经落地：

**多源摄入的深化**
- Office 从"能抽文本"变成**能预览 + 按结构索引**：Word 高保真渲染；Excel 处理合并单元格/多级表头、按行索引成自描述记录并抽取公式；PowerPoint 用真实版式渲染引擎，按幻灯片索引并纳入**演讲者备注**、SmartArt 与图表文字，幻灯片图片登记为视觉证据。
- 聊天引用可**定位回 Word 段落 / Excel 行**（Office 无 PDF 几何，改用内容锚定）。

**检索**
- **修复中日韩检索**：FTS5 默认分词器不切分 CJK，导致词组中间的词永远搜不到——这是全局性缺陷，不限于某种格式。

**书写**
- Markdown 编辑器换为 Milkdown/Crepe，补齐 `[[双链]]`、公式、自动保存、标题优先新建。
- Agent 可**读取并提出精准修改**（精确匹配 + 要求唯一 + 不做模糊回退，参考已上线编程 agent 的结论）。

**组织**
- 集合与来源支持**拖拽排序**；散落笔记回到树的根级（Obsidian 模型），不再塞进虚构的 Unfiled 文件夹。
- 可把来源从侧栏**拖进输入框**作为引用。

**数据安全（本方案未覆盖）**
- 笔记**镜像为纯 `.md` 文件**到用户指定目录 → 任何同步工具可直接搬运，且不锁定。
- **数据库快照备份与恢复**（`VACUUM INTO`，可定期），覆盖只存在于库里的集合分类与聊天记录。
