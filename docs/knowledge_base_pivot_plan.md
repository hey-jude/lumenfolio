...
**视觉 IA → 需要 live 迭代(下一轮)：**
- **P1-d 侧栏图标栏 + 面板** 与 **自适应中央舞台(首页即对话)**：是对 `WorkspaceSidebar.vue`(已有"折叠 rail"范式)与三栏布局的**大改**,与现有范式冲突、无法离线验证、应按 mockup→确认→实现→实测 的方式做。盲改核心导航/布局风险高、收益不可验证,故留作下一轮带 app 的 live 迭代。
- **现状即可用**:无文档时右侧对话已能全库问答并标注模式,功能闭环成立;视觉首页/侧栏为体验增强。
- **风险：** 内核低(增量、向后兼容);视觉 IA 中(需 live)。
