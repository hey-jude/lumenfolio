...
- **笔记镜像为纯 `.md` 文件**，存放在你指定的文件夹。把它指向 iCloud、Dropbox、坚果云或 WebDAV 挂载点，笔记就能被带离这台机器——**我们不写一行同步代码**，也没有锁定，因为它就是 Markdown。即使数据库丢了，笔记仍在，并能恢复回应用。
...
**多设备：目前是备份，不是同步。** 数据库是唯一的真相来源，应用**不读取外部对 `.md` 的改动**——在别的编辑器里改了某篇笔记，下次在应用内保存它时会覆盖掉那次改动。集合分类、聊天记录与索引也只存在于产生它们的那台机器上。因此**不要让两个 Lumenfolio 同时指向同一个已同步的笔记文件夹**。真正的双向同步需要冲突解决，是另一个量级的工程，目前刻意没有做。

**换新机器**：把新安装指向同一个笔记文件夹，用「从文件夹恢复」拉回笔记；再恢复一份数据库快照，把集合分类与聊天记录一并带过去。
